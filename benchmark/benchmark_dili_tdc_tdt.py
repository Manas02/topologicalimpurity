"""Benchmark DT vs TDT for DILI - TDC ADME task"""
...
def evaluate_models(
    ...
    topo_clf = TopologicalDecisionTreeClassifier(
        max_depth=15, mol_net_threshold=sim_threshold
    )
    ...
    metrics = {
        ...
        "Topological_Decision_Tree_Balanced_Accuracy": balanced_accuracy_score(
            y_test, topo_pred
        ),
...
for model_fp in ["rdkit", "maccs", "morgan2", "morgan3"]:
    for molnet_fp in ["rdkit", "maccs", "morgan2", "morgan3"]:
        for sim_threshold in [0.5, 0.7, 0.9, 0.95]:
            ...
            y_test = np.array(
                [
                    int(float(test_graph.nodes[i]["categorical_label"]))
            ...
            # Evaluate models
            logger.critical(
                f"STARTING EVAL...{file_name}|{model_fp=}|{molnet_fp=}|{sim_threshold=}"
            )
            ...
            metrics_df.to_csv("benchmark_dili_tdc_tdt_result.csv", index=False)
